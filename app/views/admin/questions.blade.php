							...
							<tbody>
								@foreach( $questions as $question )
									<tr>
										...
										<td>{{{ str_limit( $question->description, 80, '...' ) }}}</td>
										<td>
											@if( $question->active )
										...
										<td>
											{{ link_to_route('questions.delete', 'remove', [ 'id' => $question->id ]) }} | 
											@if( $question->active )
												{{ link_to_route('questions.desactivate', 'desactivate', [ 'id' => $question->id ]) }}
											@else
												{{ link_to_route('questions.activate', 'activate', [ 'id' => $question->id ]) }}
											@endif
										</td>
									</tr>
								@endforeach
